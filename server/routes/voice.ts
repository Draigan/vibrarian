    ...
    try {
      ...
      const normalizedMime =
        typeof mimeType === "string" && mimeType.length > 0
          ...
          : "audio/webm";
      const extension =
        normalizedMime.split("/")[1]?.split(";")[0]?.trim() || "webm";
      ...
      console.log(
        "Attempting transcription with key suffix:",
        process.env.OPENAI_API_KEY?.slice(-6)
      );
      console.log(
        "Attempting transcription with project:",
        process.env.OPENAI_PROJECT_ID
      );

      try {
        const debugPath = path.resolve(
          `logs/latest-clip.${extension === "ogg" ? "ogg" : extension}`
        );
      ...
      if (typeof data.text === "string" && data.text.trim().length > 0) {
        ...
        const segments = (data as { segments?: Array<{ text?: string }> })
          .segments;
      ...
      console.log(
        "Transcription request using key suffix:",
        process.env.OPENAI_API_KEY?.slice(-6)
      );
      console.log(
        "Transcription request using project:",
        process.env.OPENAI_PROJECT_ID
      );
      ...
      if (
        "segments" in data &&
        Array.isArray(
          (data as { segments?: Array<{ text?: string }> }).segments
        )
      ) {
        console.log(
